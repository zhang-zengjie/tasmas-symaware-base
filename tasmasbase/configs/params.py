from tasmasbase.probstlpy.benchmarks.common import inside_rectangle_formula, outside_rectangle_formula
import numpy as np
from tasmasbase.commons.agent import Agent
from tasmasbase.commons.assigner import Assigner
from tasmasbase.probstlpy.systems.linear import LinearSystem
...
def get_assigner(N, n):
    ...
    sll = [{"A": go_home(tll[0], 10, 2, HOME_A, 'BACK TO T-A'),
            "B": go_home(tll[0], 10, 2, HOME_B, 'BACK TO T-B'),
            "C": go_home(tll[0], 10, 2, HOME_C, 'BACK TO T-C'),
            "D": go_home(tll[0], 10, 2, HOME_D, 'BACK TO T-D')}
          ]

    A = Agent(sys, phi_0, N, x0=np.array([5, 5]), Q=Q, R=R, ub=4, name='A')
    B = Agent(sys, phi_0, N, x0=np.array([15, 5]), Q=Q, R=R, ub=5, name='B')
    C = Agent(sys, phi_0, N, x0=np.array([25, 5]), Q=Q, R=R, ub=6, name='C')
    D = Agent(sys, phi_0, N, x0=np.array([35, 5]), Q=Q, R=R, ub=7, name='D')

    agents = {"A": A, "B": B, "C": C, "D": D}
    TA = Assigner(agents)

    return TA, (tlg, slg), (tll, sll)