...
class LinearSystem(NonlinearSystem):
    """
    A linear discrete-time system with additive disturbance of the form
    ...
    where
        ...
        - :math:`w_t \in \mathbb{R}^n` is a additive disturbance input,
    ...
    :param mu: A ``(n,1)`` (optional) numpy array representing the mean vector
    :param Sigma: A ``(n,n)`` (optional) numpy array representing the variance matrix
    """
    def __init__(self, A, B, C, D, mu=None, Sigma=None):
        self.n = A.shape[1]
        self.m = B.shape[1]
        self.p = C.shape[0]
        ...
        if not (mu is None):
            assert mu.shape == (self.n,), "mu must be an (n,1) matrix"
        if not (Sigma is None):
            assert Sigma.shape == (self.n, self.n), "Sigma must be an (n,n) matrix"
