...
class STLTree(STLFormula):
    ...
    def __init__(self, subformula_list, combination_type, timesteps, name=None):
        ...
        self._calculate_interval(subformula_list, timesteps)
        self._calculate_prob(subformula_list)
    ...
    def _calculate_interval(self, formula_list, time_steps):

        interval_lower = [formula_list[i].interval[0] + time_steps[i] for i in range(len(formula_list))]
        interval_upper = [formula_list[i].interval[1] + time_steps[i] for i in range(len(formula_list))]
        self.interval = [min(interval_lower), max(interval_upper)]
        self.ts = list(range(self.interval[0] + 1, self.interval[1] + 1))

    def _calculate_prob(self, formula_list):

        self.prob = max([formula.prob for formula in formula_list])
